...
|------|-----------|-------------|
| Opportunity Detection | Daily 6:00 AM | LinkedIn job opportunity scanning with clickable job links |
...
Daily opportunity detection reports now include:
- **Clickable job titles** that link directly to LinkedIn job search results
- **Real LinkedIn URLs** based on job title, location, and work type
- **Filtered results** showing recent postings (last week)
...
### Cleanup Commands:
```bash
# Run reports cleanup script
python scripts/cleanup_reports.py

# Archive old test files
find reports -name "test_*" -exec mv {} reports/archive/test_files/ \;
